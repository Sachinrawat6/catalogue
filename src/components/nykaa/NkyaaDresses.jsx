...
const NkyaaDresses = () => {
  const nykaa_dresses = JSON.parse(localStorage.getItem("nykaaListingData")) || [];

  if (nykaa_dresses.length === 0) {
    return <p className="text-center mt-4 text-xl text-red-400 animate-pulse">No data available !</p>;
  }

  // 🟢 Extracting headers dynamically
  const headers = Object.keys(nykaa_dresses[0]);

  // 🔥 Filter only those rows where "Pack Contains" is "1 Dress"
  const filteredDresses = nykaa_dresses.filter(item => item["Pack Contains"] === "1 Dress" ||item["Pack Contains"] === "1 Kaftan" );
  ...
  const deleteSelectedData = () => {
    ...
    const updatedData = nykaa_dresses.filter((_, index) => !selectedItems.includes(index));
