...
const NykaaJackets = () => {
  const nykaa_jackets = JSON.parse(localStorage.getItem("nykaaListingData")) || [];

  if (nykaa_jackets.length === 0) {
    return <p className="text-center mt-4 text-xl text-red-400 animate-pulse">No data available !</p>;
  }

  // 🟢 Extracting headers dynamically
  const headers = Object.keys(nykaa_jackets[0]);

  // 🔥 Filter only those rows where "Pack Contains" is "1 Dress"
  const filteredDresses = nykaa_jackets.filter(item => item["Pack Contains"] === "1 Jacket" || item["Pack Contains"] === "1 Coat" || item["Pack Contains"] === "1 Shrug");
  ...
  const deleteSelectedData = () => {
    ...
    const updatedData = nykaa_jackets.filter((_, index) => !selectedItems.includes(index));
