...
const NykaaSkirts = () => {
  const nykaa_skirts = JSON.parse(localStorage.getItem("nykaaListingData")) || [];

  if (nykaa_skirts.length === 0) {
    return <p className="text-center mt-4 text-xl text-red-400 animate-pulse">No data available !</p>;
  }

  // 🟢 Extracting headers dynamically
  const headers = Object.keys(nykaa_skirts[0]);

  // 🔥 Filter only those rows where "Pack Contains" is "1 Dress"
  const filteredDresses = nykaa_skirts.filter(item => item["Pack Contains"] === "1 Skirt");
  ...
  const deleteSelectedData = () => {
    ...
    const updatedData = nykaa_skirts.filter((_, index) => !selectedItems.includes(index));
