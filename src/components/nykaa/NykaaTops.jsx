...
const NykaaTops = () => {
  const nykaa_tops = JSON.parse(localStorage.getItem("nykaaListingData")) || [];

  if (nykaa_tops.length === 0) {
    return <p className="text-center mt-4 text-xl text-red-400 animate-pulse">No data available !</p>;
  }

  // 🟢 Extracting headers dynamically
  const headers = Object.keys(nykaa_tops[0]);

  // 🔥 Filter only those rows where "Pack Contains" is "1 Dress"
  const filteredDresses = nykaa_tops.filter(item => item["Pack Contains"] === "1 Top");
  ...
  const deleteSelectedData = () => {
    ...
    const updatedData = nykaa_tops.filter((_, index) => !selectedItems.includes(index));
