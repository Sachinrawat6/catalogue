...
const generateAjioShirtListing = (selectedData) => {
  const csvHeaders = [
    "Style Code",
    "Style Description",
    "Item SKU",
  ...
  const csvData = selectedData.filter((product)=>product.style_type==="Shirt" ||product.style_type.includes("Shirt"))
  .flatMap((product) =>
    sizes.map((size) => {
      ...
      if (product.color && product.color.trim() !== "") {
        colorShade =
          product.color.trim().toLowerCase() === "pastel"
            ? "Light"
            : "Dark";
      }
  ...
  link.download = "ajio_shirt_listing.csv";
  document.body.appendChild(link);
  link.click();
  document.body.removeChild(link);
};

export default generateAjioShirtListing;
