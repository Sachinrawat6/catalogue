...
const generateAjioShurgsListing = (selectedData) => {
  const csvHeaders = [
    "Style Code",
    "Style Description",
    "Item SKU",
  ...
  const csvData = selectedData.filter((product)=>product.style_type==="Shrug" ||product.style_type.includes("Shrug")  )
  .flatMap((product) =>
    sizes.map((size) => {
  ...
  link.download = "ajio_shurgs_listing.csv";
...
export default generateAjioShurgsListing;